...
using UnityEngine;
public class CreatureCoroutineController : MonoBehaviour
{

    static CreatureCoroutineController _singleton;
    static Dictionary<string,IEnumerator> _routines = new Dictionary<string,IEnumerator>(100);

    [RuntimeInitializeOnLoadMethod( RuntimeInitializeLoadType.BeforeSceneLoad )]
    static void InitializeType ()
    {
        _singleton = new GameObject($"#{nameof(CreatureCoroutineController)}").AddComponent<CreatureCoroutineController>();
