...
public class EActionRangedApproachPlayer : BTLeaf
{
    private NavMeshAgent agent;
    private float angularSpeed = 720f; //deg/s
    private float acceleration = 100f; //max accel units/sec^2

    public EActionRangedApproachPlayer(string _name, EnemyAIContext _context ) : base(_name, _context)
    {
    ...
    public override NodeState Evaluate() 
    {
        ...
        } else if(Vector3.Distance(enemyContext.player.transform.position, enemyContext.enemyTransform.position) < 20 || enemyContext.animator.inAttack)
        {
            // Made it to player
            OnParentExit();
