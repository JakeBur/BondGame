Shader "Unlit/TransparentGlobe"
{
    ...
    SubShader
    {
        Tags { "RenderType"="GlobeBuffer" "RenderQueue"="Transparent"}
        LOD 50
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float fresnel = dot(normalize(forward), i.normal);
                float tanFresnel = tan(fresnel);

                return fixed4(lerp(col, _InteriorColor, fresnel * _InteriorColorStrength), saturate(_Opacity + (1 - (tanFresnel * tanFresnel))));
                //return fixed4(fixed3(1, 1, 1), 1 - fresnel);
            }
            ENDCG
        }
    }
